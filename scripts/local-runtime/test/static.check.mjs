...
assert.ok(processRuntime.indexOf('auditDevelopmentProcessEnvironments(environments, declarations)') < processRuntime.indexOf("'scripts/local-runtime/src/service-development.mjs'"), 'all selected owner environments must be audited before service spawn')
assert.ok(processRuntime.indexOf('prebuildDevelopmentOwners(root, manifest.owners)') < processRuntime.indexOf("'scripts/local-runtime/src/service-development.mjs'"), 'selected owners must finish a fresh build before service watchers start')
const serviceDevelopment = read('scripts/local-runtime/src/service-development.mjs')
assert.doesNotMatch(serviceDevelopment, /spawnCommand\(['"]pnpm/u, 'service watcher supervision must not add package-manager lifecycle wrappers')
assert.match(serviceDevelopment, /operator-interrupt/u, 'service watcher supervision must normalize intentional stops')
assert.match(serviceDevelopment, /unexpected-exit/u, 'service watcher supervision must preserve unexpected failures')
